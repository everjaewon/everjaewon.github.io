...
⑷ Paging에 대해서 설명하시오
<br><br><br><br>
⑴<br>물리적 주소 공간은 하드웨어에 의해 제공되는 주소 공간으로, 다시 말해 컴퓨터 내부의 메모리 공간을 주소로 사용하는 것이다. 절대 주소라고도 불린다. 논리적 주소는 실제 존재하는 주소가 아니라 프로세스가 자기 메모리를 보는 시점을 나타낸 것으로, 가상 주소라고도 한다. 사용자 프로그램은 논리적 주소에 대해 다루고, 실제 물리적 주소를 보는 일은 절대 없다. 명령어를 translate하는 과정에서 linking을 통해 논리적 주소에 접근하고, 여기서 loading 과정을 거치면서 실제 물리적 주소에 접근하게 된다.<br><br><br>
⑵<br>Fixed Partitioning은 메인 메모리를 파티션이라 불리는 겹치지 않는 영역들로 나누는 메커니즘으로, 각 파티션들의 크기는 동일할수도, 아닐수도 있다(like 안철수 엌ㅋ). 그러나 메인 메모리에서의 사용은 비효율적이라 할 수 있는데, 파티션들의 크기가 가변적이지 않고 고정되어 있기 때문에, 파티션보다 작은 size의 프로그램이 load될 경우, 파티션의 잉여 size를 사용할 수 없는 '내부 단편화'가 발생할 수 있다. 크기가 서로 다른 파티션들을 이용하면 해당 문제점을 완화할 수 있으나, 완전히 해결하지는 못한다.<br><br>
