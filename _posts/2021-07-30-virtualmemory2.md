...
⑴ thrashing에 대한 설명<br>
⑵ copy-on-write 설명<br>
⑶ memory-mapped file 설명
<br><br><br><br>
⑴<br>프로세스가 물리적 메모리에 충분한 수의 페이지를 갖고 있지 않을 때 page fault의 발생률이 급격하게 상승하는데, 이 page fault를 처리해야 하기 때문에 CPU 활용률을 낮추게 되고, 이를 운영체제는 멀티프로그래밍의 수준이 낮으니 높여야 한다고 판단하여 또다른 프로세스를 시스템에 추가한다. 물리적 메모리가 부족해 발생한 문제인데 상황이 더 악화되는 것이다. 스래싱이란, 이런 상황에서 각 프로세스가 페이지를 메모리 안팎으로 계속 이동, 교체시켜 정작 프로그램 처리 속도는 확 떨어지는 현상을 말한다.<br><br>
스래싱을 해결하기 위해 OS는 프로세스의 사용 중인 페이지들 모음을 working set, 일정 수의 페이지 참조를 window로 설정하고, 이를 기반으로 '가장 최근의 window에서 참조된 페이지의 수'를 프로세스의 working-set size로 설정하여, 이 working-set size들의 합이 전체 요구 프레임 수를 넘어섰을 때(즉, 스래싱이 발생할 때) 프로세스 中 하나를 중지하고 교체한다. 이는 멀티프로그래밍 수준을 가능한 높게 유지하면서 스래싱을 방지할 수 있게 해준다.<br><br><br>
⑵<br>Copy-on-Write는 부모 프로세스와 자식 프로세스가 초기부터 메모리 內의 동일한 페이지들을 공유하고, 두 프로세스 중 하나가 해당 공유 페이지를 수정하는 경우에만 페이지를 복사하는 방법이다. 오직 수정되는 페이지만 복사를 수행하기 때문에 프로세스 생성이 더 효율적이라는 특징을 갖는다. <br><br><br>
⑶<br>Memory mapped file은 메모리 안의 페이지에 disk block을 매핑함으로써 파일 입출력이 보통의 메모리 접근처럼 여겨지도록 하는 방법이다. 파일과 메모리의 접근 방식이 동일하기 때문에 복사가 덜 일어난다. 파일의 첫 읽기는 demand paging을 이용해 이루어지고, 파일의 페이지 크기만큼의 부분을 file system에서는 실제 페이지로 읽게 되면서, 이후부터의 파일 읽기/쓰기 작업은 일반적인 메모리 접근처럼 여겨지게 된다. read, write 시스템 콜을 이용할 필요 없이 메모리를 통해 파일의 입출력을 처리함으로써, 파일 접근을 보다 단순화한다는 장점을 갖게 된다.
